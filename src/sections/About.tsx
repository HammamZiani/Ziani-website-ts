...
export function About() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      cards.forEach((card) => {
        ...
        tl.to(imageWrap, { clipPath: "inset(0% 0 0 0)", duration: 1.4, ease: "expo.inOut" })
          .to(imageInner, { scale: 1, duration: 1.8, ease: "expo.out" }, "-=1.1")
          .from(contentItems, { y: 20, opacity: 0, duration: 0.8, stagger: 0.1, ease: "power3.out" }, "-=1.0")
      })
    }, sectionRef)
    return () => ctx.revert()
  }, [locale, mounted])
  ...
  return (
    <SectionLight>
      <div id="baths" ref={sectionRef} className="w-full max-w-screen-2xl">
...
function BathCard({ img, titleKey, descKey, num, t }: { img: string; titleKey: string; descKey: string; num: string; t: (k: any) => string }) {
  return (
    <article className="bath-card group flex h-full flex-col">
      <div className="card-image-wrap relative mb-6 aspect-[3/4] max-h-[550px] overflow-hidden bg-gray-100 shadow-sm">
        <img src={img} alt={t(titleKey)} className="card-image-inner h-full w-full object-cover" />
        <span className="absolute left-4 top-4 z-20 font-secondary text-[0.5rem] tracking-[0.3em] text-white mix-blend-difference">{num}</span>
      </div>
      <div className="flex flex-grow flex-col">
        <h3 className="card-content-item mb-2 font-primary text-2xl uppercase tracking-tighter text-blue-900 lg:text-3xl">{t(titleKey)}</h3>
        <p className="card-content-item max-w-[200px] font-secondary text-[0.7rem] uppercase leading-relaxed tracking-[0.15em]">{t(descKey)}</p>
