...
export default function BookingStep1({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 max-h-[40vh] lg:max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
        {formulesData.map((pkg) => (
          <button
            ...
            className={cn(
              "flex items-center justify-between p-5 border transition-all duration-300 cursor-pointer",
              selectedFormula?.id === pkg.id
                ? "border-black bg-black text-white"
                : "border-black/10 bg-transparent hover:border-black",
            )}
          >
      ...
      <Button
        variant={selectedFormula ? "solid" : "outline"}
